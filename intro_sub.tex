...
EC2)~\cite{hung} to within private networks~\cite{imai}, and even to
other mobilde devices~\cite{serendipity}.
%
All of these efforts seek intelligent ways to enable mobile device
developers to leverage the computing capabiliteis of more powerful
computing resources over the network.\\
%
This proposal presents a novel framework which addresses the challenge
...
%
Since heterogeneous computing is essentially the offloading of workloads
to different accelerators on the host platform, I believe it is natural
to extend this concept to support accessing componenets that are not
on-board, but rather across the network on remote resources.
%
...
service discovery and configuration using \lq\lq Social Area
...
heterogeneous platforms, but ir currently does not support access to
accelerators across the network.
%
Hence, the first contribution of our work is the ability to remotely
offload and execute OpenCL kernels from a mobile device to either nearby
computing nodes or to cloud resources.
%
Although the proposed OpenCL-based remote offloading for mobile
platforms is not the first to proposed the concept of offloading
heterogeneous workloads over the network (i.e. Remote CUDA~\cite{rcuda},
Virtual OpenCL~\cite{vocl}), the design is the first to consider this
approach where offloading happens between a mobile device and a virtual
...
computation.
%
The developers is able to offload computation to one or more of these
...
accelerator as if it would be for a regular accelerator located on-board
the local mobile device.
%
The proposed framework automatically offloads the computation securely
to the cloud, manages the grasfer of necessary state and returns the
reslut back to the application.
%
...
discovery mechanism and they assume the availability of remote ocmputing
resources with fixed endpoints.
%
I advocate a dunamic approach where eligible computing resources are
dicovered at runtime, allowing for a more flexible design.
%
By using IP multicast-based discovery, the system periodically locates
computing resources available within their networks.\\
%
I also investigated the conditions where offloading is more beneficial
than local processing by strategicaly selecting four types of workloads:
Sobelfilter, matrix multiplication, hidden Markov mode, and {\it N}-body
...
offloading.
%
The experiments show that it is more beneficial to offload computation
in cases of high computation intensity (i.e. high
computation-to-communication ratio), but in case of low computation
